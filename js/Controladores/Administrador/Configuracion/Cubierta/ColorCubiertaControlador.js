app.controller("ColorCubiertaControlador", function($scope, $http, $q, CONFIG, $rootScope, datosUsuario, $window, $filter, $location)
{   
    $scope.grupo = [];
    $scope.color = [];
    $scope.nuevoGrupo = [];
    ...
    $scope.AbrirGrupoColor = function(operacion, objeto)
    {
        ...
        else if(operacion == "Agregar")
        {
            ...
            $scope.nuevoGrupo.TipoGrupo.TipoGrupoId =  "1";
        ...
        $('#colorCubiertaModal').modal('toggle');
    };
    ...
    $scope.AgregarMuestrarioColor = function()    
    {
        AgregarGrupoColorCubierta($http, CONFIG, $q, $scope.nuevoGrupo).then(function(data)
        {
            ...
            $('#mensajeConfigurarCubiertaColor').modal('toggle');
